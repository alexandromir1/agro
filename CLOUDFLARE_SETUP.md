...
4. Проверьте логи:
   ...
   - Если видите "OpenAI API error: 429" или "quota" → превышен лимит/закончились средства
   - **Важно:** При ошибках квоты/биллинга сайт автоматически использует локальную симуляцию (fallback), чтобы демо продолжало работать

## Fallback режим

Если OpenAI API недоступен (квота, биллинг, ошибка сети), сайт автоматически переключается на локальную симуляцию результатов. Это гарантирует, что демо всегда работает, даже если AI временно недоступен.
